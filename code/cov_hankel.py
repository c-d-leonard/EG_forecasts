...
def get_DeltaSigma_covs(params, rp_bin_edges, rp_bin_c, lens, src, endfilename):
    ...
    pk = [ scipy.integrate.simps(pk_ofz_arr[ki, :] * dNdzl, z) for ki in range(len(k)) ] # Units (MPc/h)^3
...
def get_cov_DeltSig_gm(params, HT, k, pk, DPi2_l, ShapeNoise, ShotNoise, vol, rp_bin_edges, lens, src):
    ...
    #plt.figure()
    #plt.loglog(r_bin, np.diag(cov_ggkk_bin)/vol, 'o', label='ggkk cov term')
    #plt.loglog(k, p_kappa + ShapeNoise , 'm', label='Pkk + shapenoise')
    #plt.legend()
    #plt.savefig('../plots/covggkk_DSgm_'+lens+'_'+src+'_Aug2024.pdf')
    #plt.close()
    ...
    """plt.figure()
    plt.loglog(k, p_g + ShotNoise, 'b', label='Pgg+1/ng')
    #plt.loglog(k, p_kappa + ShapeNoise , 'm', label='Pkk + shapenoise')
    plt.legend()
    plt.savefig('../plots/powerspectra_terms_pgg_'+lens+'_'+src+'_Aug2024.png')
    plt.close()
    
    plt.figure()
    plt.loglog(k, p_g + ShotNoise, 'b', label='Pgg+1/ng')
    plt.loglog(k, p_kappa + ShapeNoise * SigC2 , 'm', label='P_kappa + ShapeNoise * SigC2')
    plt.legend()
    plt.savefig('../plots/powerspectra_terms_both_'+lens+'_'+src+'_Aug2024.png')
    plt.close()
    
    plt.figure()
    plt.loglog(r_bin, np.diag(cov_gkgk_bin)/vol, 'o', label='gkgk cov term')
    #plt.loglog(k, p_kappa + ShapeNoise , 'm', label='Pkk + shapenoise')
    plt.legend()
    plt.savefig('../plots/covgkgk_DSgm_'+lens+'_'+src+'_Aug2024.pdf')
    plt.close()
...
def W_ofPi_lensing(params, lens, src):
    ...
    zs_int = [[ scipy.integrate.simps(np.asarray(Sig_inv_ofPi[pi][zli]) * dNdzs, zs) for zli in range(len(zl))] for pi in range(len(Pi))] # Units pc^2 / (Msol h)
    
    # Integrate then over lens distribution
    zl_int = [ scipy.integrate.simps(zs_int[pi] * dNdzl, zl) for pi in range(len(Pi))] # Units pc^2 / (Msol h)
...
def DeltaPi_gm(params, lens, src):
	...
	DeltaPi1 = scipy.integrate.simps(W, Pi) # I guess then this output is in Mpc/h
	
	DeltaPi2 = scipy.integrate.simps(W**2, Pi) # and this one too
